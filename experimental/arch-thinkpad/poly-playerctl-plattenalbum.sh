...
if [[ $playerctlstatus ==  "" ]]; then
    ...
    echo "%{A1:playerctl --player=plattenalbum,%any pause:}%{A6:playerctl next:}%{A8:playerctl previous:}`playerctl metadata title | cut -d " " -f1-5`%{A}%{A}%{A}"
else
    echo "%{A1:playerctl --player=plattenalbum,%any play:}%{A}"
fi
